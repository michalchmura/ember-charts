...
import U from 'npm:underscore';

var statuses = ['interested', 'prospect', 'submitted', 'technical', 'face-to-face', 'offer', 'placement', 'rejected'];
var statuses_colors = {
  interested: '#00B5AD', 
  prospect: '#767676', 
  submitted: '#2185D0', 
...
const TimeSeriesChartComponent = ChartComponent.extend(LegendMixin,
  ...
  groupedBarAttrs: Ember.computed( 'xTimeScale', 'xGroupScale', 'barWidth', 'yScale',
    ...
    var selected_seed_color = this.get('selectedSeedColor');
    var secondary_color = this.get('getSecondarySeriesColor');
  ...
  legendItems: Ember.computed('xBetweenSeriesDomain', 'xWithinGroupDomain',
    ...
    var result = this.get('xBetweenSeriesDomain').map((d, i) => {
      ...
      // Bar legend items
      var color;
      if(U.contains(statuses, d)) {
        color = statuses_colors[d];
      } else { 
        color = this.get('getSecondarySeriesColor')(d, i);
      }
      var res = {
        stroke: color,
        fill: color,
  ...
  updateBarGraphic: function() {
    ...
    groups.selectAll('rect')
      .style('fill', this.get('groupedBarAttrs.color'))
